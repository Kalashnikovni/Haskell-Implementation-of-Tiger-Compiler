...
travVar (SimpleVar s) = 
  do lvl <- lookUpLvl s
     ...
     return (SimpleVar s)
travVar (FieldVar v p) = 
  do v' <- travVar v
     return (FieldVar v' p)
travVar (SubscriptVar v e) = 
  do v' <- travVar v
     e' <- travExp e
...
travExp (VarExp v p) =
  -- | FIXME: ¿Por qué hay un adder?
  do v' <- adder (travVar v) (pack $ printPos p)
     return (VarExp v' p)
travExp (CallExp s args p) =
  do args' <- mapM travExp args
     return (CallExp s args' p)
travExp (OpExp l op r p) = 
  do l' <- travExp l
     r' <- travExp r
     return (OpExp l' op r' p)
travExp (RecordExp es s p) = 
  do es' <- mapM (\(s', e) -> travExp e >>= return . (s',)) es
     return (RecordExp es' s p)
travExp (SeqExp es p) = 
  do es' <- mapM travExp es
     return (SeqExp es' p)
travExp (AssignExp v e p) = 
  -- | FIXME: ¿Por qué hay un adder?
  do v' <- adder (travVar v) (pack $ printPos p)
     e' <- travExp e
     return (AssignExp v' e' p)
travExp (IfExp c t Nothing p) = 
  do c' <- travExp c
     t' <- travExp t
     return (IfExp c' t' Nothing p)
travExp (IfExp c t (Just e) p) = 
  do c' <- travExp c
     t' <- travExp t
     e' <- travExp e
...
travExp (ForExp s e lo hi body p) = 
  do lo' <- travExp lo
     hi' <- travExp hi
...
travF :: (Escapator m) => (Symbol, [(Symbol, Escapa, Ty)], Maybe Symbol, Exp, Pos) -> 
                          m (Symbol, [(Symbol, Escapa, Ty)], Maybe Symbol, Exp, Pos)
travF (name, params, res, body, p) = 
  do (body', params') <- bulkInsert (map (\(a, b, _) -> (a, b)) params) 
                                    (do body' <- travExp body
                                        ds'   <- mapM (\(s, _, ty) -> 
                                                        do mb <- lookup s
                                                           case mb of
...
travDecs [] m = m
travDecs ((FunctionDec ls) : xs) m =
  do ls' <- up (mapM travF ls)
     travDecs xs m
travDecs ((VarDec name esc typ init p) : xs) m = 
  do init' <- travExp init
...
data Estado = S { lvl :: Int, env :: Env} deriving Show

-- Este estado va a llevar el nivel y una lista de entornos, que son todos
-- los entorno intermedios...
data SEstado = Step { lvlP :: Int, envP :: Env, msgP :: [String]} deriving Show
...
instance Escapator Stepper where
  ...
  update name esc = 
    do addMsgM ("Update de la variable " ++ unpack name)
       est <- get
  ...
  insert name esc m = 
    do addMsgM ("Agregamos la variable " ++ unpack name)
       old <- get
