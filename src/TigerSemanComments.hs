...
-- en el entorno.
-- + Para realizar correctamente la detección de ciclos se utiliza el algoritmo
...
--   los tipos, para esto van a necesitar una función |transTy :: Ty -> w Tipo|
...
--  dados |(s : Symbol)|, |(t : Tipo)|, y un simbolo |s'|, busque en la tabla el
--  tipo que tiene asignado |s'| y lo recorre buscando referencias al record |s|
--  y si la encuentra la reemplace por el tipo |t|. (ver |autoRef|).
----------------------------------------
